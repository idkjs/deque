...
let rec list_gen = (size, m) =>
  if (size <= 0) {
    ...
    m >>= (x => rest >>= (xs => return([Buffer.L2R(x), ...xs])));
  };
...
type holy('a, 'preference, 'color) =
  | Holy(
      not_empty('a, 'b, 'preference, (_, _) as 'hole_loc),
      triple(
        'b,
        'b,
        [< | `green | `red] as 'color,
...
and only_green:
  type a. m(a) => m(triple(a, a, [ | `green], only, nh, nh, nh, not_rev)) =
  e =>
    Nest(
      () =>
...
and only_red:
  type a. m(a) => m(triple(a, a, [ | `red], only, nh, nh, nh, not_rev)) =
  e =>
    Nest(
      () =>
        buffer_ge5(e)
  ...
  e =>
    Nest(
      () =>
        ...
        @ (
          right_red(e)
          >>= (k => return([@implicit_arity] Path(HOLE, Triple_rev(k))))
        )
        @ (
          buffer_ge7(e)
          >>= (
            ...
            (G_path(left)) =>
              ...
              >>= (
                ([@implicit_arity] Path(y, k)) =>
                  return(
                    [@implicit_arity]
                    Holy([@implicit_arity] Pair_left_sym(left, y), k),
                  )
              )
          )
        )
            ...
            ([@implicit_arity] Path(y, k)) =>
              ...
              >>= (
                (G_path(right)) =>
                  return(
                    [@implicit_arity]
                    Holy([@implicit_arity] Pair_right_sym(y, right), k),
                  )
              )
          )
        )
...
and left_green:
  type a. m(a) => m(triple(a, a, [ | `green], left, nh, nh, nh, not_rev)) =
  e =>
    Nest(
      () =>
        (
...
and left_red:
  type a. m(a) => m(triple(a, a, [ | `red], left, nh, nh, nh, not_rev)) =
  e =>
    Nest(
      () =>
        buffer_ge5(e)
...
and right_green:
  type a. m(a) => m(triple(a, a, [ | `green], right, nh, nh, nh, not_rev)) =
  e =>
    Nest(
      () =>
        (
...
and right_red:
  type a. m(a) => m(triple(a, a, [ | `red], right, nh, nh, nh, not_rev)) =
  e =>
    Nest(
      () =>
        buffer_eq2(e)
  ...
  e =>
    Nest(
      () =>
        ...
        @ (gr_deque(e) >>= ((GR_deq(d)) => return(S(Rev(d))))),
    )
...
let fold_left_to_list = t =>
  List.rev(
    Deque.Deckrev.fold_left((xs, x) => [x, ...xs], [], Obj.magic(t)),
  );
